...
class TurtleBot3(object):
	...
	def lineup(self):
		t0 = rospy.Time.now().to_sec()
		current_dist = 0
		mid_point_angle = np.radians(180 - self.tb_angle - np.abs(self.aruco_angle))
		dist = np.abs((self.aruco_dist * math.sin(self.tb_angle)) / mid_point_angle)
		while current_dist < dist:
			
			self.vel.linear.x = 0.05
			self.twist_pub.publish(self.vel)
			t1 = rospy.Time.now().to_sec()
			current_dist = 0.05 * (t1 - t0)
			self.rate.sleep()
		# Forcing Turtlebot to stop
		self.vel.linear.x = 0
		self.twist_pub.publish(self.vel)
		self.rate.sleep()
		#print(np.abs(self.tb_angle))
